...
We compile and standardize five widely used datasets: HadCRUT5 [@HadCRUT5], GISTEMP [@GISTEMP], NOAAGlobalTemp [@NOAAGlobalTemp], Berkeley Earth [@BerkeleyEarth], and ERA5 [@ERA5]. To strengthen interpretation, we integrate the Oceanic Nino Index [@SST_Part1; @SST_Part2] and annotate El Nino and La Nina phases in the visualizations, making short-term ENSO variability easier to interpret alongside long-term warming.
...
The next utility converts wide-format data to long format. This is required for `GISTEMP`, where each row corresponds to one year and monthly values are stored in separate columns. The `longseries` function reshapes this structure into one row per month.
